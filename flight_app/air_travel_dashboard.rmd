...
delay_plot <- ggplot(data=delay_cause_timeseries, aes(x=delay_cause_total_year.year, y=value, group = Cause, color = Cause)) + geom_line() + geom_point(size=1, shape = 21, fill = "white") + labs(x="Year", y= "Total Annual Delay Minutes") + scale_y_continuous(labels = comma) + theme(axis.text.x=element_text(angle=90, hjust=1))
...
weekday_plot <- ggplot(data=weekday_agg, aes(x=Var1,y=Freq)) +
