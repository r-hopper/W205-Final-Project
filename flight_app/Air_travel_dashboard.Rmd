...
highprop <- ggplot(data=high_delay_prop, aes(x=origin, y=delayprop, color=origin, fill=origin)) + geom_bar(stat = "identity") + ggtitle("From Top 100 Airports by Departure Volume") + labs(x="Origin", y="Proportion of Delayed Flights") + scale_y_continuous(labels=comma)  + theme(axis.text.x=element_text(angle=90, hjust=1), plot.title= element_text(color="#666666", size=12, hjust=.7))  + theme(legend.position="none")
highprop
ggplotly(highprop)
```
